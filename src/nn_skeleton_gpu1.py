...
class ModelSkeleton:
  ...
  def _add_interpretation_graph(self):
    ...
    with tf.variable_scope('interpret_output') as scope: #11111
      preds = self.preds
    ...
    with tf.variable_scope('bbox') as scope:  #222222222222222222
    ...
    with tf.variable_scope('IOU'):   #3333333333333
    ...
    with tf.variable_scope('probability') as scope: #44444444444444444444
  ...
  def _add_train_graph(self):
    ...
    #--------------------------------------tk step 1-------------------------------
    #print("self.loss  : {}\ntf.trainable_variables() : {}\n".format(self.loss, tf.trainable_variables()))
    grads_vars = opt.compute_gradients(self.loss, tf.trainable_variables())
    #print("gard_vars : {}".format(grads_vars))
    

    with tf.variable_scope('clip_gradient') as scope:
      for i, (grad, var) in enumerate(grads_vars):
	#--------------------------------------tk-------------------------------
        #grads_vars[i] = (lambda grad: 0 if grad is None 
	#		else tf.clip_by_norm(grad, mc.MAX_GRAD_NORM), var)
        #print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
	#print("clip_by_norm is {} var is {}".format(grad, var))
	grads_vars[i] = (tf.clip_by_norm(grad, mc.MAX_GRAD_NORM), var)
	#print("grad_vars[i] is {}".format(grads_vars[i]))
    #--------------------------------------tk step 2-------------------------------
...
#      print("inputs?? {}".format(inputs))
#      inputs_temp1 = tf.cast(inputs, tf.int8)
#      inputs_temp1 = tf.stop_gradient(inputs_temp1)
#      inputs_temp2 = tf.bitcast(inputs_temp1, tf.qint8)
#      inputs_temp2 = tf.stop_gradient(inputs_temp2)
#      inputs_revised = tf.bitcast(inputs_temp2, tf.quint8)
#      inputs_revised = tf.stop_gradient(inputs_revised)
#      print("revised_input?? {}".format(inputs_revised))
#      print("                                             ***                                   ")
#      print("kernel?? {}".format(kernel))
#      kernel_temp1 = tf.cast(kernel, tf.int8)
#      kernel_temp1 = tf.stop_gradient(kernel_temp1)
#      kernel_temp2 = tf.bitcast(kernel_temp1, tf.qint8)
#      kernel_temp2 = tf.stop_gradient(kernel_temp2)
#      kernel_revised = tf.bitcast(kernel_temp2, tf.quint8)
#      kernel_revised = tf.stop_gradient(kernel_revised)
#      print("revised_kernel?? {}".format(kernel_revised))
#      print("                                             ***                                   ")

#      before_conv, min_conv, max_conv = tf.nn.quantized_conv2d(
#          inputs_revised, kernel_revised, min_input, max_input, min_filter, max_filter, [1, stride, stride, 1], 
#	  padding=padding, out_type=tf.qint32, name='convolution')
     
#      print("before_conv?? {}".format(before_conv)) 
#      conv = tf.bitcast(before_conv, tf.float32)
#      print("conv?? {}".format(conv))
#      print("                                             ***                                   ")
...
#      print("-----------------------------------------------------------------------------------")
      ...
      print("                                             ***                                   ")
      print("kernel?? {}".format(kernel))
      kernel_revised = self._quantize(kernel, 4)
      print("revised_kernel?? {}".format(kernel_revised))
      print("                                             ***                                   ")
 ...
 #------------------------------------tk----------------
  def _quantize(self, inputs, k):
        n = float(2**k - 1)

	out = tf.divide(tf.round(tf.multiply(inputs, n)) , n) 
        #print("layer name, k is ({}, {}) input : {}".format(layer_name, k, inputs))
	#print("--------------------------!!-------------")
	#print("out.get_shape() is {}".format(out.get_shape()))
